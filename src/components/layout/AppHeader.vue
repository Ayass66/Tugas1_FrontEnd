...
<template>
 <header class="sticky top-0 z-50 w-full border-b bg-background/95 backdropblur">
 <div class="container mx-auto flex h-16 items-center justify-between px4">
 ...
 <!-- Aksi kanan: search + user menu -->
 <div class="flex items-center gap-2">
 <!-- Search input kecil -->
 <div class="relative hidden sm:block">
 <Search class="absolute left-2.5 top-1/2 -translate-y-1/2
...
import { BookOpenCheck, Search, User, LayoutDashboard,
 LogOut, BookOpen, Home } from 'lucide-vue-next'
const router = useRouter()
const kataCariHeader = ref('')
// Data user — Bab 4 akan dari auth store Pinia
const namaUser = ref('Ahmad Fauzi')
const isPustakawan = ref(false)
const inisialUser = computed(() => {
 return namaUser.value.split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2)
})
const navItems = [
...
function cariDariHeader() {
 ...
 router.push({ name: 'katalog', query: { q: kataCariHeader.value } })
